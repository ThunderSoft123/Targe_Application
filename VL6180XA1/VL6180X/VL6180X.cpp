...
{
   i2cWriteForVL6180X(i2c,ADDRESS_DEFAULT,&value,reg,sizeof(value));
...
void VL6180X::writeReg16Bit(uint16_t reg, uint16_t value)
{
  ...
  buffer[1] = value & 0xff;
  i2cWriteForVL6180X(i2c,ADDRESS_DEFAULT,buffer,reg,sizeof(value));
  // Wire.beginTransmission(address);
  // Wire.write((reg >> 8) & 0xff);  // reg high byte
  // Wire.write(reg & 0xff);         // reg low byte
...
{
  uint8_t value = 0xff;
  i2cReadForVL6180X(i2c,ADDRESS_DEFAULT,&value,reg,1);
  return value;
...
{
  uint16_t value;
  i2cReadForVL6180X(i2c,ADDRESS_DEFAULT,(uint8_t *)&value,reg,sizeof(value));
  return value;
  // uint16_t value;
