...
#define read_font_line(__char, __line)  font[((uint16_t)(__char)) * FONT_CHAR_HEIGHT+(__line)]
...
void GraphicsDisplay::character_(int x, int y, char ch,int colour) { 
  uint8_t temp;
  uint8_t pos,t;
    //blitbit(column * 8, row * 8, 8, 8, (char*)&(FONT8x8[value - 0x1F][0]));
  if ((x > (width() - FONT_CHAR_WIDTH)) || (y > (height() - FONT_CHAR_HEIGHT))) {
    return;
  }
...
void GraphicsDisplay::lcd_display_char(uint16_t hwXpos, //specify x position.
